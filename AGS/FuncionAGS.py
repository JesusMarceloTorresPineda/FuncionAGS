...
def generate_pulation(minimo, maximo, tamaño_poblacion): # Poblacion inicial
   while len(initial_population) < tamaño_poblacion:
      individuo = random.randint(minimo,maximo)
      print(individuo)
...
def tranformar_bin_int(param_j):
   d=[]
   for x in param_j:
      ls=[]
      a = bin_to_int(x[0:6])
...
if __name__ == "__main__":
   ...
   for i in range(50):
      ...
      re_translate = tranformar_bin_int(mutation_data)
      print(re_translate[0], " ---- ", re_translate[1])
      porsiacaso= maximo-(random.randint(minimo,maximo))
      print("porsi ", porsiacaso)
      if re_translate[0]> maximo:
         re_translate[0]= maximo-porsiacaso
      if re_translate[1]>maximo:
         re_translate[1]= maximo-porsiacaso
